...
```mermaid
flowchart LR
  U[User device] -->|ai-input| C[AI session channel]
  C -->|durable create| H[(History store)]
  C -->|versioned create/append/update| V[(Version store)]
  A[Agent worker] -->|ai-output stream| C
  P[Provider] -->|tokens/tools/reasoning| A
  C -->|fanout + rollup| U
  C -->|presence| M[Other devices]
  C -->|complete event| N[Push notifications]
```
...
sequenceDiagram
  ...
  participant Sockudo
  participant Agent
  participant Store as History/version store

  Client->>Sockudo: ai-input create
  Sockudo->>Store: persist input
  Agent->>Sockudo: ai-turn-start
  Agent->>Sockudo: ai-output create
  loop provider stream
    Agent->>Sockudo: message.append
    Sockudo-->>Client: rolled-up append fanout
  end
  Agent->>Sockudo: ai-turn-end complete
  Sockudo->>Store: final aggregate visible to history
```
