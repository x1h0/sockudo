...
If `policy.features.enable_user_authentication` is on, the signin event (`pusher:signin` on V1, `sockudo:signin` on V2) must use signed `auth` with signed `user_data`.

## Origin Validation

Per-app `policy.channels.allowed_origins` can restrict WebSocket origins.
