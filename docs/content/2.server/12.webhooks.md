...
Webhooks are live notifications. They are not retained presence-history queries and they are not a historical database.
...
Webhooks are defined per-app in the `policy.webhooks` field:
...
- live webhook notifications:
  fire when the transition happens now
- current presence membership:
  use `GET /apps/{appId}/channels/{channelName}/users`
- retained historical presence events:
  use `GET /apps/{appId}/channels/{channelName}/presence/history`

If you need to answer questions like "who joined during the last hour?" from retained Sockudo state, use presence history instead of replaying webhook deliveries.
...
Or simply don't define any `policy.webhooks` in your app configuration.
