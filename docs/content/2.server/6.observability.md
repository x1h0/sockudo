...
`GET /usage` returns memory statistics snapshot from server process context. Can be disabled via `http_api.usage_enabled: false` or `HTTP_API_USAGE_ENABLED=false`.
