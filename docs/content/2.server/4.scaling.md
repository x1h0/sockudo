...
- Node restart: the current in-memory presence-history stream is lost, a new `stream_id` starts on that node, and previously retained presence-history events from that node are no longer available. Live presence state can repopulate, but retained history before the restart is gone.
