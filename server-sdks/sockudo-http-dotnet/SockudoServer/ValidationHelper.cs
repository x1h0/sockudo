...
namespace SockudoServer
{
    ...
    {
        /// <summary>
        /// A regular expression to check that a channel name is in a format allowed and accepted by Sockudo.
        /// </summary>
        public static Regex CHANNEL_NAME_REGEX = new Regex(@"\A[a-zA-Z0-9_=@,.;\-]+\z", RegexOptions.Singleline);

        /// <summary>
        /// The maximum length of a channel name allowed by Sockudo.
        /// </summary>
        public static int CHANNEL_NAME_MAX_LENGTH = 164;

        /// <summary>
        /// A regular expression to check that a socket_id is in a format allowed and accepted by Sockudo.
        /// </summary>
        public static Regex SOCKET_ID_REGEX = new Regex(@"\A\d+\.\d+\z", RegexOptions.Singleline);

        /// <summary>
        /// The maximum event batch size accepted by Sockudo
        /// </summary>
        ...
        /// <param name="options">The current <see cref="ISockudoOptions"/>.</param>
        /// <exception cref="EventDataSizeExceededException">If the size of <paramref name="data"/> is greater than the expected.</exception>
        /// <remarks>Note: the size of the data field is only validated if the <c>ISockudoOption.BatchEventDataSizeLimit</c> is specified.</remarks>
        internal static void ValidateBatchEventData(string data, string channelName, string eventName, ISockudoOptions options)
        {
